...
export default function NotesYearSelectionScreen() {
  const navigation = useNavigation();

  return (
    <View style={styles.root}>
      <TopNavbar title="Notes" showBack={false} />
      <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
        <Text style={styles.heading}>Select Academic Year</Text>
        <Text style={styles.subHeading}>Find Notes by Your Academic Year</Text>
        {YEAR_LEVELS.map((item) => (
          <TouchableOpacity
            key={item.id}
            activeOpacity={0.9}
            style={styles.card}
            onPress={() => navigation.navigate("NotesSubjectSelect", { yearLevel: item.level })}
          >
